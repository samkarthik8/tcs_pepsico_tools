...
const TRINO = {
    ...
    port: 443,
    // Confirm these names if catalog/schema in the supplied URL were placeholders.
    catalog: "catalog",
    schema: "schema",
...
function trinoRequest(pathname, body, password) {
    return new Promise((resolve, reject) => {
        const request = https.request({
            hostname: TRINO.host, port: TRINO.port, path: pathname,
            method: body ? "POST" : "GET",
            // Matches SSLVerification=NONE in the supplied JDBC URL.
...
async function runThReconciliationQuery(password) {
    ...
    while (true) {
        ...
        if (page.data?.length) rows.push(...page.data.slice(0, maxRows - rows.length));
        if (!page.nextUri || rows.length >= maxRows) break;
        const nextUrl = new URL(page.nextUri);
        page = await trinoRequest(`${nextUrl.pathname}${nextUrl.search}`, null, password);
    }
    return {columns: columns.map((column) => column.name), rows, truncated: rows.length >= maxRows};
}
...
app.whenReady().then(() => {
    const splash = new BrowserWindow({
        width: 600, height: 300, frame: false, alwaysOnTop: true,
