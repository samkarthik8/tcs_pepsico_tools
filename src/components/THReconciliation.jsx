...
export default function THReconciliation() {
    ...
    const runTrinoQuery = async () => {
        if (!window.thReconciliation?.runQuery) {
            setQueryError(
                "This feature is available only in the installed EXE."
            );
            return;
        }
        ...
        try {
            const result = await window.thReconciliation.runQuery();
            ...
            setQueryError(
                error.message ||
                "Unable to retrieve data from Trino."
            );
        } finally {
    ...
    const exportQueryCsv = () => {
        ...
        link.href = url;
        link.download =
            `TH_Reconciliation_${new Date().toISOString().slice(0, 10)}.csv`;
            ...
            <div
                ...
                <div className="w-full bg-black/30 p-8 rounded-2xl border border-white/10 shadow-2xl backdrop-blur">
                    ...
                    <div className="flex justify-between items-center mb-4 gap-4">
                        ...
                        <button
                            onClick={() =>
                                navigator.clipboard.writeText(QUERY_STRING)
                            }
                            className="shrink-0 text-sm bg-[#E4002B] hover:bg-[#c70024] text-white px-4 py-2 rounded-lg shadow-lg transition font-medium"
                        >
                            Copy Query
                        </button>
                    </div>
                    ...
                    <div className="w-full overflow-x-auto rounded-2xl border border-white/20">
                        ...
                        <div
                            ...
                            <span>
                                ...
                                of{" "}
                                {queryRows.length.toLocaleString()}{" "}
                                retrieved rows
                            </span>

                            <button
                                onClick={exportQueryCsv}
                                className="bg-[#E4002B] hover:bg-[#c70024] px-4 py-2 rounded-lg font-bold flex items-center justify-center gap-2"
                            >
                                <Download size={18}/>
                                Export query output CSV
                            </button>
                        ...
                        <table className="w-full text-left bg-white/10">
                            ...
                            {displayedRows.map((row, rowIndex) => (
                                ...
                                >
                                    {queryColumns.map((_, columnIndex) => (
                                        <td
                                            key={columnIndex}
                                            className="px-5 py-3"
                                        >
